...
float4 PixelShaderExample(PSInput PS) : COLOR0
{
    ...
    float4 moviecol = tex2D(Sampler0, PS.TexCoord);
    
    float2 uvOffset = tex2D(Sampler0, float2(Time*99,0)).rg;
    uvOffset.x *= 0.001;
    uvOffset.y *= 0.003;
    
    moviecol.r = tex2D(Sampler0, uv + uvOffset ).r;
    moviecol.g = tex2D(Sampler0, uv + uvOffset).g;
    moviecol.b = tex2D(Sampler0, uv + uvOffset + float2(-0.01*tex2D(Sampler0, float2(uv.x/100.0,uv.y + Time*99)).r, 0) ).b;
    
    
    moviecol.rgb = lerp(moviecol.rgb, float3(dot(moviecol.r, 0.33),0,0), 0.1);
    
	float4 finalColor = float4(moviecol);
